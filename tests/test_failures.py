...
class FailureTest(unittest.TestCase):
    ...
    def test_permission_version_duplicate_and_invariant(self):
        ...
        current=self.service.get_item(self.item["id"],"viewer")
        self.service.add_measurement(current["id"],{"measured_at":"2026-09-25T08:00:00+00:00","dose":4,"conclusion":"normal","measured_by":"lab-a"},"recorder",'radiation_officer')
        for target in STATES[2:-1]: current=self.service.transition(current["id"],target,current["version"],"reviewer",TRANSITION_ROLES[target][0])
