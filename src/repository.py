...
class Repository:
            ...
            self.conn.executescript(f"""
                ...
                CREATE TABLE IF NOT EXISTS measurements (
                    ...
                    conclusion TEXT NOT NULL
                        CHECK(conclusion IN ('normal','exceeded','inconclusive')),
    ...
    def latest_measurements_map(self, item_ids: List[int]) -> Dict[int, Dict[str, Any]]:
        if not item_ids:
            return {}
        placeholders = ",".join("?" for _ in item_ids)
        with self._lock:
            rows = self.conn.execute(
                f"""SELECT m.* FROM measurements m
                    JOIN (SELECT item_id, MAX(id) AS max_id FROM measurements
                          WHERE item_id IN ({placeholders}) GROUP BY item_id) last
                    ON m.id = last.max_id""",
                tuple(item_ids),
            ).fetchall()
